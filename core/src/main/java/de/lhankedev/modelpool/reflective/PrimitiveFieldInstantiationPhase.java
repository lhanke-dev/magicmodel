...
import static de.lhankedev.modelpool.model.AttributeDefinition.OBJECT_REFERENCE_PREFIX;
...
public class PrimitiveFieldInstantiationPhase implements ModelCreationPhase {
    ...
    private void instantiatePrimitiveFields(final CreatedModelPoolObject createdModelPoolObject) {
        final List<AttributeDefinition> primitiveAttributes = createdModelPoolObject.getMmObjectDefinition()
                .getAttributes().stream()
                .filter(this::isPrimitiveAttribute)
                .collect(Collectors.toList());

        final Object objectInstance = createdModelPoolObject.getCreatedObject();
