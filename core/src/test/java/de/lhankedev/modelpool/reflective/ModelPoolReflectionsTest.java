...
import de.lhankedev.modelpool.assertion.CarAssertion;
...
class ModelPoolReflectionsTest {
    ...
    private final ModelPoolReflections cut = new ModelPoolReflections();
    ...
    void testClazzForName() {
        final Optional<Class<?>> reflectionsClazz = cut.clazzForName(ModelPoolReflections.class.getCanonicalName());
        Assertions.assertThat(reflectionsClazz)
                .isPresent()
                .containsSame(ModelPoolReflections.class);
    }
    ...
    void openResource() throws IOException {
        try (final InputStream in = cut.openResource(
                "de/lhankedev/modelpool/resources/testClassPathResourceProvider.txt")) {
